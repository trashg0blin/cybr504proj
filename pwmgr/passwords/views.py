...
def delete_password(request,pk):
    if request.method =='POST':
        try:
            ...
            messages.add_message(request,messages.SUCCESS,'Password hint was successfully updated!')
            return HttpResponseRedirect(reverse('passwords:home'))
        ...
        return render(request, 'passwords/delete_password.html',{'password_obj':passwordHintObj})
...
def update_hints(request, pk):
    if request.method == 'POST':
        try:
            ...
            messages.add_message(request, messages.SUCCESS,'Password hints successfully updated!')
            return HttpResponseRedirect(reverse('passwords:home'))
        except Exception as err:
            print(err)
            return HttpResponse('Some error occurred')
        ...
        if passwordHintObj.created_by_id != request.user.id:
            raise PermissionDenied()
        return render(request, 'passwords/update_hints.html',{'password_obj':passwordHintObj})
...
def update_password(request, pk):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS,'You have successfully updated your password hint!')
...
def create_generated_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.SUCCESS,'You have successfully generated and stored password for %s !' % formData['password_belongs_to'])
